...
import { createInternalRadication } from "../../../services/api";
...
export default function RadicarPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Llamada a la API
      const response = await createInternalRadication(formData);
  ...
  return (
    <div className={styles.wrap}>
      <div className={styles.heading}>
        <div>
          <h1 className={styles.h1}>Radicación Interna</h1>
          <p className={styles.sub}>Documentos entre dependencia</p>
        </div>
